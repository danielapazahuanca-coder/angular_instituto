...
<div class="ticket" id="comprobante">
  ...
  <div class="body">
    ...
    <div class="row" *ngIf="datos.tipo_pago === 'mensual'">
      <span class="label">Duración:</span>
      <span class="value">{{ datos.duracion_meses }} meses</span>
    </div>
    <hr class="dashed">
    ...
    <!-- SECCIÓN DE DETALLE DE PAGO SEGÚN TIPO -->
    
    <!-- CASO: PAGO MENSUAL -->
    <ng-container *ngIf="datos.tipo_pago === 'mensual'">
      <div class="row">
        <span class="label">Inscripción:</span>
        <span class="value text-end">{{ datos.monto_inscripcion | currency:'Bs ':'symbol':'1.2' }}</span>
      </div>
      <div class="row" *ngIf="datos.monto_reserva > 0">
        <span class="label">Reserva:</span>
        <span class="value text-end "> {{ datos.monto_reserva | currency:'Bs ':'symbol':'1.2' }}</span>
      </div>
      <div class="row" *ngIf="datos.descuento > 0">
        <span class="label">Descuento:</span>
        <span class="value text-end text-danger">- {{ datos.descuento | currency:'Bs ':'symbol':'1.2' }}</span>
      </div>
    </ng-container>

    <!-- CASO: PAGO TOTAL -->
    <ng-container *ngIf="datos.tipo_pago === 'total'">
      <div class="row">
        <span class="label">Inscripción:</span>
        <span class="value text-end">{{ datos.monto_inscripcion | currency:'Bs ':'symbol':'1.2' }}</span>
      </div>
